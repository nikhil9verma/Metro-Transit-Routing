  ...
  return (
    <div>
      <div className="header">
        ...
        <h1>Metro <span>Transit</span> Routing</h1>
      </div>
      <RouteForm stops={stops} onSearch={handleSearch} loading={loading} />
      {error && <div className="error-bar">⚠ {error}</div>}
      <div className="main-layout">
        <div className="sidebar">
          <div className="sidebar-title">
            {journeys.length > 0 ? `${journeys.length} Routes Found` : 'Route Options'}
          </div>
          {journeys.length === 0 && !loading && (
            <div className="empty-state">
              <div className="empty-icon">🗺</div>
              <p>Select origin &amp; destination<br/>then click Find Route</p>
            </div>
          )}
          {journeys.map((j, i) => (
            <RouteCard
        ...
        <MapView
          stops={stops}
          selectedJourney={selectedIdx !== null ? journeys[selectedIdx] : null}
        />
      </div>
    </div>
  );
